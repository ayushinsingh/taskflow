...
export const TaskCard: React.FC<TaskCardProp> = ({ taskId, index }) => {
  ...
  const totalCompletedTasks = task.subTaskIds.reduce((total, current) => (state.subTasks.entities[current].isCompleted ? total + 1 : total), 0);
  if (!task) return null;
    ...
    <Draggable draggableId={taskId} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-zinc-900 p-4 rounded-md border border-zinc-800 shadow-sm transition-shadow ${snapshot.isDragging
        ...
        >
          ...
          <div className="mt-2.5 flex items-center justify-between text-xs text-zinc-500 font-medium tabular-nums">
            <span className={`mt-3 inline-block text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded ${activeColorMap[task.priority]}`}>
              {task.priority}
            </span>
            <span className="text-zinc-400">
              {totalCompletedTasks} <span className="text-zinc-600">/</span> {task.subTaskIds.length}
            </span>
          </div>
        </div>
      )}
